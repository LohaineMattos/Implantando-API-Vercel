...
function Modal({ receita, onClose, onSave, isEditing, isCreate }) {
  ...
  const handleSave = () => {
    if (onSave) {
      ...
      if (isEditing) {
        onSave(receita.id, updatedData); // ✅ passa o ID na edição
      } else {
        onSave(updatedData); // ✅ criação
      }
    }
    onClose();
  };
...
function deleteReceita(id) {
  axios
    ...
    .then((response) => {
      ...
      window.location.reload(); // Força atualização da lista após deletar
    })
    .catch((error) => {
...
function updateReceita(id, data) {
  ...
  axios
    ...
    .then((response) => {
      ...
      window.location.reload(); // Atualiza lista após editar
    })
    .catch((error) => {
  ...
  const handleCreateReceita = (newData) => {
    axios.post('https://receitasapi-b-2025.vercel.app/receitas', newData)
      .then((response) => {
        ...
        window.location.reload(); // Atualiza após criar
      })
      .catch((error) => {
